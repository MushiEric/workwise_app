...
void main() {
  group('Menu visibility (role/permission based)', () {
    testWidgets('AppDrawer shows HR when user has "show hrm dashboard" permission', (tester) async {
      ...
      final container = ProviderContainer(overrides: [permissionCheckerProvider.overrideWithValue(checker)]);
      expect(container.read(permissionCheckerProvider).hasPermission('show hrm dashboard'), isTrue);

      // compute visible menus using same logic as AppDrawer to assert expectation
      final visible = appMenus
          .where((m) => m.requiredPermissions == null || m.requiredPermissions!.isEmpty || checker.hasAnyPermission(m.requiredPermissions!))
          .map((m) => m.title)
          .toList();
      // ignore: avoid_print
    ...
    testWidgets('AppDrawer hides HR when user lacks "show hrm dashboard" permission', (tester) async {
      final user = domain.User(
        id: 2,
        name: 'Bob',
        roles: [
          Role(id: 5, name: 'staff', permissions: [Permission(id: 1, name: 'show pos dashboard')]),
        ],
      );
    ...
    testWidgets('IndexPage hides Project tile when permission missing and shows when present', (tester) async {
      ...
      final userNoProject = domain.User(
        id: 3,
        name: 'NoProj',
        roles: [Role(id: 6, name: 'user', permissions: [Permission(id: 1, name: 'show pos dashboard')])],
      );
      ...
      final visibleNoProj = appMenus
          .where((m) => m.requiredPermissions == null || m.requiredPermissions!.isEmpty || checkerNo.hasAnyPermission(m.requiredPermissions!))
          .map((m) => m.title)
          .toList();
      ...
      final userWithProject = domain.User(
        id: 4,
        name: 'ProjUser',
        roles: [Role(id: 7, name: 'company', permissions: [Permission(id: 5, name: 'show project dashboard')])],
      );
      ...
      final visibleYesProj = appMenus
          .where((m) => m.requiredPermissions == null || m.requiredPermissions!.isEmpty || checkerYes.hasAnyPermission(m.requiredPermissions!))
          .map((m) => m.title)
          .toList();
      expect(visibleYesProj.contains('Project'), isTrue);
    });

    testWidgets('AppDrawer shows Jobcard when user has jobcard dashboard permission and hides otherwise', (tester) async {
      // has 'manage jobcard dashboard' (backend may use either name)
      final userWithJobcard = domain.User(
        id: 8,
        name: 'JobUser',
        roles: [Role(id: 9, name: 'company', permissions: [Permission(id: 11, name: 'manage jobcard dashboard')])],
      );
      ...
      final userNoJobcard = domain.User(
        id: 9,
        name: 'NoJob',
        roles: [Role(id: 10, name: 'staff', permissions: [Permission(id: 12, name: 'show pos dashboard')])],
      );
